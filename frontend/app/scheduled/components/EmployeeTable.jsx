...
import { setFacility } from "#/(redux)/slices/facilitySlice";
...
import { setScheduled } from "#/(redux)/slices/scheduledSlice";
...
export default function FacilityTable({ facilities }) {
  ...
  const onViewPress = async (e) => {
    const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASEURL}/scheduled/${e.target.id}`);
    dispatch(setScheduled(res.data));
    router.push(`/scheduled/${e.target.id}`);
  };
  ...
  const onDeletePress = async (e) => {
    try {
      const res = await axios.delete(`${process.env.NEXT_PUBLIC_API_BASEURL}/scheduled/${e.target.id}`);
      router.refresh();
  ...
  return (
    <div className="">
      ...
      <div className={`flex justify-between items-center mb-4 px-1`}>
        <h2 className="font-extrabold text-2xl">List of Schedules</h2>
