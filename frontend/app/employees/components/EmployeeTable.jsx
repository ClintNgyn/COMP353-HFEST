...
export default function FacilityTable({ facilities }) {
  const columnDefinitions = [
    ...
    { key: "id", label: "EmployeeID" },
    { key: "personId", label: "PersonID" },
    { key: "medicareNum", label: "medicareNum" },
  ];
  ...
  const onViewPress = async (e) => {
    const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASEURL}/employees/${e.target.id}`);
    dispatch(setEmployee(res.data));
    router.push(`/employees/${e.target.id}`);
  };
  ...
  const onDeletePress = async (e) => {
    try {
      const res = await axios.delete(`${process.env.NEXT_PUBLIC_API_BASEURL}/employees/${e.target.id}`);
      router.refresh();
  ...
  return (
    <div className="">
      ...
      <div className={`flex justify-between items-center mb-4 px-1`}>
        <h2 className="font-extrabold text-2xl">List of Employees</h2>
