    ...
    try {
      ...
      if (facility) {
        ...
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_API_BASEURL}/infected/${facility.id}
          `,
          finalFacility,
        );
      ...
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_BASEURL}/infected
        `,
        finalFacility,
      );
        ...
        {(onClose) => (
          <>
            <div className="flex border-b-2 items-center justify-between  mb-2">
              <ModalHeader>{!facility ? "Create a new Infection" : "Update Infection"}</ModalHeader>
            </div>
            <ModalBody>
