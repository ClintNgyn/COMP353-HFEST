...
import { setInfected } from "#/(redux)/slices/infectedSlice";

export default function FacilityTable({ facilities }) {
  const columnDefinitions = [
    { key: "actions", label: "Actions" },
    { key: "personId", label: "personID" },
    { key: "infectionType", label: "infectionType" },
    { key: "infectionDate", label: "infectionDate" },
  ];
  ...
  const onViewPress = async (e) => {
    const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASEURL}/infected/${e.target.id}`);
    dispatch(setInfected(res.data));
    router.push(`/infections/${e.target.id}`);
  };
  ...
  const onDeletePress = async (e) => {
    try {
      const res = await axios.delete(`${process.env.NEXT_PUBLIC_API_BASEURL}/infected/${e.target.id}`);
      router.refresh();
  ...
  return (
    <div className="">
      ...
      <div className={`flex justify-between items-center mb-4 px-1`}>
        <h2 className="font-extrabold text-2xl">List of Infections</h2>
